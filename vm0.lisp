  ...
  `(defun execute (instruction stack sp)
     (let ((control '(:continue)))
       (macrolet
           ((safe-pop ()
              `(if (<= sp 0)
                   (return-from execute (values (list :trap :stack-underflow)))
                   (aref stack (decf sp))))
            (safe-push (val)
              `(if (>= sp (length stack))
                   (return-from execute (values (list :trap :stack-overflow) sp))
                   (progn (setf (aref stack sp) ,val) (incf sp))))
            (safe-pick ()
              '(let* ((i (safe-pop)) (stack-pos (- sp 1 i)))
                (if (< stack-pos 0)
                    (return-from execute (values (list :trap :stack-out-of-bounds)))
                    (safe-push (aref stack stack-pos)))))
            (with-1 (a &body body)
              `(let ((,a (safe-pop)))
                 ,@body))
